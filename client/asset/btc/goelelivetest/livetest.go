...
// Regnet tests expect the BTC test harness to be running plus
// ElectrumX test server 'on top'. No background mining.
//
// See also: github.com/dev-warrior777/go-electrum-client/btc/test_harness
// See also: github.com/dev-warrior777/go-electrum-client/btc/rpc_test
// See also: github.com/dev-warrior777/go-electrum-client/cmd/mkwallet
// See also: github.com/dev-warrior777/go-electrum-client/cmd/rmwallet
// See also: github.com/dev-warrior777/go-electrum-client/cmd/goele
//
// Sim harness info:
// -----------------
// The btc harness has one rpc wallet, alpha, name=""
// We also have a Goele wallet.
// Both wallets to be well funded with confirmed UTXOs.
// Alpha mines many coins then sends some back to itself for testing.
...
func tBackendGoele(ctx context.Context, t *testing.T, cfg *Config, dir string, walletName *WalletName, blkFunc func(string)) *connectedWallet {
	t.Helper()
	...
	cloneCfg := &btc.BTCCloneCFG{
		...
		WalletInfo:          btc.WalletInfo,
		DefaultFallbackFee:  1,
		DefaultFeeRateLimit: 20,
		Segwit:              true,
		// GoeleWallet constructor overrides btc.localFeeRate = gew.walletFeeRate
	}
	gew, err := btc.GoeleWallet(cloneCfg)
	if err != nil {
		t.Fatal(err)
	}

	cm := dex.NewConnectionMaster(gew)
	err = cm.Connect(ctx)
	if err != nil {
		t.Fatalf("error connecting backend: %v", err)
	}
...
func Run(t *testing.T, cfg *Config) {
	...
	// mine 2 blocks for the first mine after goele startup to get a reply from
	// electrumX. Maybe the timout needs to be lengthened more - see how. ElectrumX
	// log shows the block update for the first mine but it never tells us.
	mine2 := func() {
		mine()
		mine()
		time.Sleep(blockWait)
	...
	t.Log("Setting up alpha/goele wallet backends...")
	rig.firstWallet = tBackend(tCtx, t, cfg, firstDir, cfg.FirstWallet, blkFunc)
	rig.secondWallet = tBackendGoele(tCtx, t, cfg, secondDir, cfg.SecondWallet, blkFunc)
	defer rig.close()
	defer rig.firstWallet.ReturnCoins(nil)
	defer rig.secondWallet.ReturnCoins(nil)
	...
	var utxos []asset.Coin
	// Goele must be funded by hand for now. Make a goele wallet using mkwallet,
	// set up btc harness + electrumX harness. Run goele.go functional test app
	// and call goele rpc test client 'getunusedaddress' to both get a funding
	// address && create an electrumX subscription to that address. Use the btc
	// harness to 'sendtoaddress <goele address> n'
	...
	tLogger.Info("Testing FundOrder #2")
	...
	// Don't check zero confs for SPV.
	...
	checkConfs := func(minN uint32, expSpent bool) {
		...
		if confs < minN {
			...
			t.Logf("expected %d confs, got %d - wallet tip probably behind electrumx", minN, confs)
			fmt.Printf("expected %d confs, got %d - wallet tip probably behind electrumx - known issue", minN, confs)
		}
		if spent != expSpent {
	...
	fmt.Printf("redemptions[0] %v\n", redemptions[0].Spends.Coin)
	fmt.Printf("redemptions[1] %v\n", redemptions[1].Spends.Coin)
	...
	// // baru
	// _, foundSecret, err := rig.secondWallet.FindRedemption(tCtx, swapReceipt.Coin().ID(), swapReceipt.Contract())
	// if err != nil {
	// 	fmt.Println("FindRedemption bombed out...", err)
	// }
	// fmt.Println(hex.EncodeToString(foundSecret))
	// // end baru
	...
	latencyQ.Wait(&wait.Waiter{
		...
		TryFunc: func() wait.TryDirective {
			...
			defer cancel()
			_, _, err = rig.secondWallet.FindRedemption(ctx, swapReceipt.Coin().ID(), swapReceipt.Contract())
			if err != nil {
				return wait.TryAgain
			}
			close(found)
	...
	_, checkKey, err := rig.secondWallet.FindRedemption(ctx, swapReceipt.Coin().ID(), swapReceipt.Contract())
	if err != nil {
	...
	// Have goele send a swap contract to the alpha address.
	setOrderValue(contractValue)
	utxos, _, _, _ = rig.secondWallet.FundOrder(ord)
	...
	// Some work to do here. Goele does not yet handle spend in the mempool
	// "txn-mempool-conflict"
	mine2() // this mine is not in the original btc livetest
